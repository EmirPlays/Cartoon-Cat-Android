...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('title/gradientBg'));
		bg.antialiasing = true;
		bg.setGraphicSize(FlxG.width, FlxG.height);
		bg.updateHitbox();
		add(bg);

		logoBl = new FlxSprite(-70, -10);
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			{
				FlxTween.tween(logoBl, {y: 3000}, 2.7, {ease: FlxEase.quintInOut});
				FlxTween.tween(cc, {y: 3000}, 2.7, {ease: FlxEase.quintInOut});
				FlxTween.tween(titleText, {y: 3000}, 2.7, {
	...
	override function beatHit()
	{
		...
		{
			case 3:
				createCoolText(['vs cartoon cat team', 'presents']);
			case 5:
				deleteCoolText();
